...
app = typer.Typer(help="Manage local LREI data.", no_args_is_help=True)
DatabaseOption = Annotated[
    Path | None,
    typer.Option("--database", "-d", help="Override the configured database path."),
]
...
def _database(database_path: Path | None) -> Database:
    ...
    return Database(database_path or settings.database_path)


@app.command("init-db")
def initialize_database(database: DatabaseOption = None) -> None:
    """Create the database schema."""
    selected_database = _database(database)
    selected_database.initialize()
    typer.echo(f"Initialized database: {selected_database.path}")


@app.command()
def add(
    message: Annotated[str, typer.Argument(help="Message to store.")],
    database: DatabaseOption = None,
) -> None:
    """Add a new entry."""
    entry = _database(database).add_entry(message)
    typer.echo(f"Added entry {entry.id}: {entry.message}")


@app.command("list")
def list_entries(database: DatabaseOption = None) -> None:
    """List entries from newest to oldest."""
    for entry in _database(database).list_entries():
        typer.echo(f"{entry.id}\t{entry.created_at}\t{entry.message}")
