    ...
    def __post_init__(self) -> None:
        ...
        if self.strong_number is not None:
            if not isinstance(self.strong_number, int):
                raise InvalidDrawError("Strong number must be an integer")
            if self.strong_number <= 0:
                raise InvalidDrawError("Strong number must be positive")

    def __post_init_strong_number_range__(self) -> None:
...
class DatasetStatistics:
    ...
    def from_dataset(cls, dataset: LotteryDataset) -> "DatasetStatistics":
        numbers = [number for draw in dataset for number in draw.numbers]
        return cls(
...
class CsvDatasetLoader:
    """Load lottery draws from a comma- or tab-delimited CSV file."""
    ...
    def load(self, path: str | Path) -> LotteryDataset:
        ...
        try:
            with file_path.open("r", encoding="utf-8", newline="") as file:
                ...
                if separator == "," and "draw_id\t" in sample.splitlines()[0]:
                    separator = "\t"
                ...
                required = {self.draw_id_column, self.numbers_column}
                ...
                for row_number, row in enumerate(reader, start=2):
                    try:
                        ...
                        date = ((row.get(self.date_column) or "").strip() or None)
                        strong_number = self._parse_strong_number(
                            row.get(self.strong_number_column)
                        )
                        ...
                        raise DatasetError(f"Invalid row {row_number}: {exc}") from exc
    ...
    def _parse_numbers(value: str) -> tuple[int, ...]:
        ...
        try:
            ...
            raise InvalidDrawError("numbers must contain integers") from exc
        return numbers

    @staticmethod
    def _parse_strong_number(value: str | None) -> int | None:
        if value is None:
            return None
        value = value.strip()
        if not value:
        ...
        try:
            ...
            raise InvalidDrawError("strong_number must be an integer") from exc
        if number <= 0:
            raise InvalidDrawError("strong_number must be positive")
        return number
...
class WalkForwardDataset:
    ...
    def __init__(self, dataset: LotteryDataset, train_size: int, test_size: int = 1) -> None:
        if train_size <= 0:
            raise ValueError("train_size must be positive")
        if test_size <= 0:
            raise ValueError("test_size must be positive")
        if train_size + test_size > len(dataset):
            raise DatasetError("train_size + test_size exceeds dataset size")
    ...
    def splits(self) -> Iterator[tuple[LotteryDataset, LotteryDataset]]:
        ...
        while start + self.test_size <= len(self.dataset):
            ...
            test = LotteryDataset(self.dataset.draws[start:start + self.test_size])
            yield train, test
